 ...
 * @param permanentCode 永久授权码，通过get_permanent_code获取
 */
exports.getCorpToken = async function (authCorpid) {
  const token = await this.ensureAccessToken()
  return this.request({
    method: 'post',
    url: 'auth/v3/tenant_access_token/',
    data: {
      app_access_token: token.accessToken,
      tenant_key: authCorpid
    },
    ignoreAccessToken: true
  })
  .then(({tenant_access_token, expire}) => {
    return {
