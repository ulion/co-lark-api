...
class API {
  ...
  async request (opts, retry = 3) {
    ...
    return this.axiosInstance.request(opts).then(async (res) => {
      ...
      if (error.response) {
        ...
        msg = error.response.data.code !== 'undefined' ? '服务器内部错误' : error.response.data
  ...
  async getAccessToken () {
    ...
    const expireTime = Date.now() + (expires_in - 10) * 1000
